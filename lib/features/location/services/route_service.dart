...
class RouteService {
  ...
  static Future<List<LatLng>> getRoutePoints(LatLng origin, LatLng destination) async {
    try {
      final String url = '$_baseUrl/json'
  ...
  static List<LatLng> _getFallbackRoute(LatLng origin, LatLng destination) {
    ...
    int numPoints = (distance * 100000).round().clamp(5, 20); // More points for longer distances
  ...
  static RouteInfo _getFallbackRouteInfo(LatLng origin, LatLng destination) {
    ...
    int estimatedTime = (distance * 2).round(); // Rough estimate: 2 minutes per km
